...
from elif_model.structures import MultilevelPixelData, PoseDataSample
...
def PackPoseInputs(results: dict, meta_keys=('id', 'img_id', 'img_path', 'category_id',
                            'crowd_index', 'ori_shape', 'img_shape',
                            'input_size', 'input_center', 'input_scale',
                            'flip', 'flip_direction', 'flip_indices',
                            'raw_ann_info', 'dataset_name'),
                 pack_transformed=True) -> dict:
    ...
    if pack_transformed and 'transformed_keypoints' in results:
    ...
    img_meta = {k: results[k] for k in meta_keys if k in results}
...
def CustomCollatePoseEstimation(batched_data):
    """Custom collate function for Pose estimation.

    Args:
        batched_data (list): A list of data samples (dict)
    Returns:
        list: The packed data samples
    """
    inputs = []
    samples = []
    for item in batched_data:
        packed_result = PackPoseInputs(item)
        input = packed_result['inputs']
        data_sample = packed_result['data_samples']    
        inputs.append(input)
